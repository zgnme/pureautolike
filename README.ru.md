...
- Человеческая траектория мыши и разброс точки клика в Chromium-сборке.
- CDP-клики через официальный Chromium `debugger` API, когда он доступен.
...
В этой GitHub-версии нет старого desktop-приложения. Есть только легкая
проверка beta/license access для будущей подписки; старого FastAPI backend,
MCP-сервера, анализа истории чатов, полноценного сетевого монитора,
SQLite-хранилища и system tray нет.

Это сделано специально: публичная версия должна быть маленьким расширением, а не
тяжелым комбайном.
